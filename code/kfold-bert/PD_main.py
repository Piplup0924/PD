...
# wandb_logger = WandbLogger(project="PD-kfold", name= "bert-10fold")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--is_train", type=utils.str2bool, default=False, help="train the EE model or not (default: True)")
    parser.add_argument("--batch_size", type=int, default=16, help="input batch size for training and test (default: 4)")
    parser.add_argument("--max_epochs", type=int, default=20, help="the max epochs for training and test (default: 20)")
    ...
    parser.add_argument("--ckpt_save_path", type=str,
                        default="/home/hutu/PersonaDataset/PD/code/kfold-bert/weight/".format(WORKING_DIR), help="ckpt_save_path")
    ...
    if args.is_train == True:
        ...
        for k in range(args.num_folds):
            ...
            ckpt_callback = ModelCheckpoint(
                ...
                filename=str(k) + "_" + args.ckpt_name + "_{epoch}_{avg_acc:.4f}",   # 模型保存名称，参数ckpt_name后加入epoch信息以及验证集分数
                monitor='avg_acc',                                      # 根据验证集上的准确率评估模型优劣
                mode='max',
                save_top_k=1,                                           # 保存得分最高的前两个模型
                verbose=True,
            )
