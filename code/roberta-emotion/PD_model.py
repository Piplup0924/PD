...
class EEModel(pl.LightningModule):
    def __init__(self, config):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(config.pretrained_path)  # "hfl/chinese-roberta-wwm-ext"

        self.processor = EEProcessor(config, self.tokenizer)
        self.model = XLMRobertaModel.from_pretrained(config.pretrained_path)
    ...
    def prepare_data(self):
        ...
        if self.use_bart:
            self.train_loader = self.processor.create_dataloader(
                train_data,train_label,batch_size=self.batch_size, shuffle=True)
            self.valid_loader = self.processor.create_dataloader(
                dev_data,dev_label,batch_size=self.batch_size, shuffle=False)
