...
class EEModel(pl.LightningModule):
    ...
    def prepare_data(self):
        ...
        [dev_data,dev_emolabel, dev_label]=[train_data,train_emolabel, train_label]
        ...
        if self.use_bart:
            self.train_loader = self.processor.create_dataloader(
                1, train_data,train_emolabel, train_label,batch_size=self.batch_size, shuffle=True)
            self.valid_loader = self.processor.create_dataloader(
                2, dev_data,dev_emolabel,dev_label,batch_size=self.batch_size, shuffle=False)
            # self.test_loader = self.processor.create_dataloader(0, dev_data,dev_label,batch_size=self.batch_size, shuffle=False)

    def forward(self, input_ids, attention_mask=None, emolabels = None):
