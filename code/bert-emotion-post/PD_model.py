...
class EEModel(pl.LightningModule):
    def __init__(self, config):
        ...
        self.classifier = nn.Linear(self.hidden_size + 5, self.num_labels)

        # self.emo_classifier = nn.Linear
        # self.emo_classifier = nn.MultiheadAttention(23, 1, batch_first=True)
        self.layer_norm = nn.LayerNorm(self.hidden_size + 5)
    ...
    def prepare_data(self):
        [train_data,train_emolabel, train_label] = self.processor.get_train_data()
        [dev_data,dev_emolabel, dev_label]=self.processor.get_dev_data()
        ...
        if self.use_bart:
            self.train_loader = self.processor.create_dataloader(
                train_data,train_emolabel, train_label,batch_size=self.batch_size, shuffle=True)
            self.valid_loader = self.processor.create_dataloader(
                dev_data,dev_emolabel,dev_label,batch_size=self.batch_size, shuffle=False)
            # self.test_loader = self.processor.create_dataloader(val_data, batch_size=self.batch_size, shuffle=False)

    def forward(self, input_ids, attention_mask=None, emolabels = None, return_dict = None, labels = None):
        ...
        # emo_atten_outputs = self.emo_classifier(emolabels, emolabels, emolabels, key_padding_mask=emolabel_attention_mask)[0]
        outputs = torch.cat((emolabels, text_outputs), dim=1)
    ...
    def training_step(self, batch, batch_idx):
        if self.use_bart:
            input_ids, attention_mask, emolabels,labels = batch
            outputs = self(input_ids = input_ids,attention_mask=attention_mask,emolabels=emolabels, 
                           labels = labels)
